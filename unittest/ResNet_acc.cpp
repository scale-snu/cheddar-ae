...
TEST_P(Testbed, ResNet20) {
  ...
  int num_test_images = g_num_test_images;
  CIFAR *cifar;
  Matrix_t output;
  Matrix_t test_data;
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--num_test_images") {
      if (i + 1 < argc) {
        try {
          g_num_test_images = std::stoi(argv[++i]);
        } catch (const std::invalid_argument& e) {
          std::cerr << "Error: Invalid number provided for --num_test_images." << std::endl;
          return 1;
        }
      } else {
        std::cerr << "Error: --num_test_images flag requires a value." << std::endl;
        return 1;
      }
    }
  }
